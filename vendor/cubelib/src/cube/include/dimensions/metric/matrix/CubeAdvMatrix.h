...
#ifndef __ADV_MATRIX_H
#define __ADV_MATRIX_H
...
public:
    ...
    virtual void setValue( Value *, cnode_id_t, thread_id_t ) = 0;
    ...
    // / For the calculation of the incl/excl values needed method.  Sum N values in the row, starting from  (cnode, thread)
    virtual Value* sumRow( cnode_id_t, thread_id_t, uint64_t ) = 0;

    // / For the calculation of the incl/excl values needed method.  Sum all values in the row
    virtual Value* sumRow( cnode_id_t ) = 0;
