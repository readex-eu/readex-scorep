...
InclusiveBuildInTypeMetric<T>::get_sev( Cnode* cnode, CalculationFlavour cnf, Sysres* sys, CalculationFlavour sf )
{
...

    {
        ...
        for ( std::vector<Sysres*>::const_iterator _siter = _locations.begin(); _siter != _locations.end(); ++_siter )
        {
            ...
            _v = ( _mapped == NULL ) ? 0 : ( ( this->adv_sev_mat )->template getBuildInValue<T>( this->calltree_local_ids[ _mapped->get_id() ], _loc->get_id() ) );
            ...
            for ( std::vector<Cnode*>::const_iterator _iter = _cnodes.begin(); _iter != _cnodes.end(); ++_iter )
            {
                ...
                for ( std::vector<Sysres*>::const_iterator _siter = _locations.begin(); _siter != _locations.end(); ++_siter )
                {
                    ...
                    T                  __v     = (
                        ...
                        ?
                        static_cast<T>( 0 ) :
                        ( this->adv_sev_mat )->template getBuildInValue<T>( this->calltree_local_ids[ _mapped->get_id() ], _loc->get_id() )
                        );
