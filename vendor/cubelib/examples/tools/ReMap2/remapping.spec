<doc>
<mirrors>
  <murl>http://www.fakeurl.de/jsc/datapool/scalasca/</murl>
</mirrors>
</doc>
 <metrics>
      <metric>
        ...
        <metric type="PREDERIVED_EXCLUSIVE">
  	    ...
  	    <cubepl>{if (
  		    ...
  		    ) {${a}=metric::time(*,*)-metric::omp_idle_threads(*,*);} else {${a}=0;}; return ${a}; }
  	    </cubepl>
              <metric type="PREDERIVED_EXCLUSIVE">
      ...
      <metric >
        ...
        <cubepl> 
	    metric::imbalance_above() + metric::imbalance_below()
	</cubepl>
          <metric  type="PREDERIVED_EXCLUSIVE">
		...
		${is_parallel}=0;
		${devidor}=0;
		while ( ( ${i} < sizeof(calculation::region::name)  ) and (  (${is_parallel} != 0) ) ) 
		{     
		  if ( ${calculation::region::name}[${i}] seq "!$omp parallel") 
		  {  
		    ${is_parallel}=1;  
		  };  		    
		}; 
		if ( ${is_parallel} != 0) 
		{  
		  ${devidor}=${cube::#threads::nonvoid};  
		}
		else
		{
		  ${devidor}=${cube::#processes::nonvoid};  
		};  

		if ( (${is_parallel} == 0) and (${calculation::sysres::rank} != 0))
		{
		  ${myvalue}=0;
		}
		else
		{
		  ${sum}=metric::fixed::execution(e) - metric::fixed::mpi(e) - metric::fixed::omp_time(e);
		  ${average} = ${sum}/${devidor};
		  ${myvalue}=(metric::execution(e,*) - metric::mpi(e,*) - metric::omp_time(e,*)) - ${average};
		};
		return pos(${myvalue}); 
	    }
		...
		else
		{
		 ...
		 if (${visitors} != ${calculation::#threads::nonvoid}) 
		 {
			...
			{
			  ${sum}=metric::fixed::execution(e) - metric::fixed::mpi(e) - metric::fixed::omp_time(e);
			  ${myvalue} =  (${sum}/${devidor}) * (${calculation::#threads::nonvoid} - 1);			
			};
		      };
		    };
		 };
