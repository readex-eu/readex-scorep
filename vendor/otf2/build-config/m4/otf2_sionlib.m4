...
dnl                    AFS_SUMMARY([sionlib support], [yes, using external via ${otf2_sionlib_config_bin} (built from trunk, version checks disabled, might produce compile and link errors.)])],
...
otf2_have_sionlib="no"
    ...
    if test "x${otf2_have_sionconfig}" = "xyes"; then
        AC_LANG_PUSH([C])
        ...
        AC_CHECK_HEADER([sion.h], [], [otf2_have_sionlib="no"; otf2_sionlib_cppflags=""])

        if test "x${otf2_have_sionlib}" = "xyes"; then
...
sion_get_current_location(42,NULL,NULL,NULL,NULL);
                           ...
                           [AC_MSG_RESULT([yes])],
                           [AC_MSG_RESULT([no])
                            otf2_have_sionlib="no"; otf2_sionlib_ldflags=""; otf2_sionlib_rpathflags=""; otf2_sionlib_libs=""])
...
AFS_SUMMARY([SIONlib support], [${otf2_have_sionlib}])
])
