...
track_free( void* ptr )
{
    size_t size = (  *( ( size_t* )( ptr - BLOCK_SIZE_SHIFT ) ) );
    if ( cubew_allocator.tracking_on != 0 )
    {
            ...
            char* message )
{
    ...
    {
        free( ptr - BLOCK_SIZE_SHIFT );
    }
#ifdef HAVE_MEMORY_TRACING
    UTILS_WARNING( "%d %d %d [CUBEW FREE] : %s %d", cubew_allocator.memory_stats.allocations_counter, cubew_allocator.memory_stats.current_nallocs, cubew_allocator.memory_stats.current_memory, message, ptr - BLOCK_SIZE_SHIFT );
#endif
}
              ...
              char*  message )
{
    ...
    if ( cubew_allocator.alloc != 0 )
    {
        ...
        if ( ptr != NULL )
        {
            memset( ptr + BLOCK_SIZE_SHIFT, 0, size * nitems );
        }
    }
    if ( ptr == NULL )
    {
               ...
               char*  message )
{
    ...
    else
    {
        if ( cubew_allocator.alloc != 0 )
        {
            ...
            memset( new_ptr + BLOCK_SIZE_SHIFT, 0, nsize );
            memcpy( new_ptr + BLOCK_SIZE_SHIFT, ptr, osize );
            cubew_free( ptr, "realloc: release old pointer" );
        }
        else
        {
            new_ptr = realloc( ( ptr != NULL ) ? ( ptr - BLOCK_SIZE_SHIFT ) : NULL, nsize + BLOCK_SIZE_SHIFT );
            ext     = 0;
            track_realloc( new_ptr, osize, nsize, ext );
        }
        new_ptr += BLOCK_SIZE_SHIFT; /* correct here, coz cubew_malloc above delivers shifter pointer*/
    }
#ifdef HAVE_MEMORY_TRACING
