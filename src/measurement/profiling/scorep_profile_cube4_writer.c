...
/**
   Returns the implicit runtime for @a node as a TAU tuple.
   This functions are given to scorep_profile_write_cube_metric.
   @param node Pointer to a node which should return the metric in second.
   @param data Ignored.
   @returns the implicit runtime of @a node.
 */

static cube_type_tau_atomic
get_modified_time_tuple( scorep_profile_node* node, void* data )
{
    cube_type_tau_atomic value;
    value.N    = node->count;
    value.Min  = ( ( double )node->inclusive_time.min ) / ( ( double )SCOREP_Timer_GetClockResolution() );
    value.Max  = ( ( double )node->inclusive_time.max ) / ( ( double )SCOREP_Timer_GetClockResolution() );
    value.Sum  = ( ( double )node->inclusive_time.sum ) / ( ( double )SCOREP_Timer_GetClockResolution() );
    value.Sum2 = ( ( double )node->inclusive_time.squares ) / ( ( double )SCOREP_Timer_GetClockResolution() * ( double )SCOREP_Timer_GetClockResolution() );
    return value;
}
