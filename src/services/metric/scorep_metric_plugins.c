...
initialize_source( void )
{
    ...
    if ( metric_plugins_initialized )
    {
        ...
        for ( uint32_t i = 0; i < num_selected_plugins; i++ )
        {
             ...
             * RTLD_NODELETE: libraries are not unloaded during dlclose().
             *                Consequently, the library's static variables
             *                are not reinitialized if the library is reloaded
             *                with dlopen() at a later time. This flag is not
             *                specified in POSIX.1-2001. Therefore we check. */
#ifdef RTLD_NODELETE
            handle = dlopen( buffer, RTLD_NOW | RTLD_NODELETE );
#else
            handle = dlopen( buffer, RTLD_NOW );
#endif

#if !defined( RTLD_NODELETE )  &&  defined( HAVE_ONLINE_ACCESS )  &&  HAVE_ONLINE_ACCESS == 1
            UTILS_WARNING( "RTLD_NODELETE not supported. Metric plugins, that use static variables, configured through Online Access might not work properly.\n" );
#endif
